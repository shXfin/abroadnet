...
export default function JourneyTimeline() {
  ...
  return (
    <>
      ...
      <div className="mt-10 grid grid-cols-2 gap-x-6 gap-y-5 sm:mt-14 sm:gap-y-10 sm:grid-cols-4 lg:grid-cols-7 lg:gap-x-3">
        {steps.map((step, i) => (
          <div key={step.label} className="relative">
            <div className="flex items-center">
              <span
                ...
                onClick={() => handleTap(i)}
                className="z-10 flex h-7 w-7 shrink-0 select-none items-center justify-center rounded-full border-2 border-navy/25 bg-paper font-mono text-xs font-bold text-navy sm:h-9 sm:w-9"
              >
                {i + 1}
              </span>
              <span className={`hidden h-[2px] flex-1 bg-navy/15 lg:block ${i === steps.length - 1 ? "opacity-0" : ""}`} />
            </div>
            <p className="mt-2 font-display text-base text-navy sm:mt-4 sm:text-lg">{step.label}</p>
            <p className="mt-1 max-w-[180px] text-xs leading-snug text-ink/60 sm:mt-2 sm:leading-relaxed">{step.copy}</p>
          </div>
        ))}
      </div>
    </>
